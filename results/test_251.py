...
for key in profile:
    print(key + str(profile[key]))import glob
import csv
import pdb
import os
import pandas as pd 
...
for server in open('server_list.txt'):				# Open the file and read each line
	subprocess.Popen(('nslookup '+server))	# Run the nslookup command for each server in the list# Script Name   : get_info_remoute_srv.py
...
if not os.path.exists('testdir'):		#  Check to see if it exists  
  os.makedirs('testdir')				#  Create the directory def get_user_input(start,end):
...
###Program gives error message and asks for new input.import time
import webbrowser
...
while(countBreaks < totalBreaks):
    ...
    countBreaks = countBreaks+1# batch_file_rename.py
...
for key in fsizedicr:       #iterating through the dictionary
    print ("Folder Size: " + str(round(fsizedicr[key]*dir_size, 2)) + " " + key)        # round function example: round(4.2384, 2) ==> 4.23# Script Name   : env_check.py
# Author        : Craig Richards

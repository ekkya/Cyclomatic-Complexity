...
for files in os.listdir(logsdir):                                       # Find all the files in the directory
	if files.endswith(".log"):                                      # Check to ensure the files in the directory end in .log
		...
		os.remove(files)                                        # Remove the original log files# Script Name	: dice.py
# Author		: Craig Richards
...
d = Die(6)
d1 = Die(6)
print d.roll(), d1.roll()# Script Name	: check_for_sqlite_files.py
# Author		: Craig Richards
...
print str(id)+'\t'+ time.encode('utf-8')+'\t'+date.encode('utf-8')+'\t'+year.encode('utf-8')+'\t'+ str(num_share)+'\t'+str(num_like)'''Simple million word count program.
...
print("List\n" + str(wordlist) + "\n")
print("Frequencies\n" + str(wordfreq) + "\n")
print("Pairs\n" + str(list(zip(wordlist, wordfreq))))# Script Name   : get_info_remoute_srv.py
...
while True:
  ...
  if row == None:
    break
  print row[0]for x in range(2,100,2):
  print(x)import pygame, sys, time
...
if not os.path.exists('testdir'):  # Check to see if it exists
    os.makedirs('testdir')  # Create the directoryimport sys
...
#-----------------------------------
  ...
  try:
    ...
    print 'Send failed!'
    print e.reason# Script Name		: recyclebin.py
# Author				: Craig Richards
